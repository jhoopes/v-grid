...
class Parser {
  parseRelationships(resource, model, included) {
    ...
    for (var relKey in resource.relationships) {
      if (!Object.hasOwnProperty.call(resource.relationships, relKey)) {
        continue;
      }
  ...
  parseJSONAPIResponse(response) {
    if (!response.data && response.meta && response.errors) {
      throw new Error("Invalid JSON:API response.  Unable to parse response");
    }

    var resources = [];
    if (Array.isArray(response.data)) {
      resources = this.parseMultipleResources(
        response.data,
        response.included,
        response.meta
      );
    ...
    if (response.meta.current_page) {
      // meta has laravel like pagination information
      resources = {
